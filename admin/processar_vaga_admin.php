<?php
// Este arquivo serve como um redirecionador para o arquivo processar_vaga.php
// Necessário para manter compatibilidade com o JavaScript existente
...
if (function_exists('logAdminAction')) {
    ...
    logAdminAction('processar_vaga_admin', "Redirecionando para processar_vaga.php - Ação: $acao, Vaga ID: $vaga_id");
}
...
switch ($acao) {
        ...
        try {
            ...
            $vaga = $db->fetch("
                SELECT v.*,
                       ...
                       v.nivel_experiencia,
                       v.tipo_contrato_id,
                       v.regime_trabalho_id,
                       v.nivel_experiencia_id,
                       CASE 
            ...
            if ($vaga) {
                ...
                // Converter valores numéricos para garantir que sejam formatados corretamente no JSON
                if (isset($vaga['salario_min'])) {
                    $vaga['salario_min'] = floatval($vaga['salario_min']);
                }
                
                if (isset($vaga['salario_max'])) {
                    $vaga['salario_max'] = floatval($vaga['salario_max']);
                }
                
                // Adicionar log para depuração
                error_log('Detalhes da vaga: ' . json_encode($vaga));
    ...
    default:
        // Para outras ações, processar diretamente aqui em vez de incluir o arquivo
        ...
        // Obter nomes dos campos para log e compatibilidade
        $tipo_contrato = '';
        $regime_trabalho = '';
        $nivel_experiencia = '';
        ...
        if (empty($erros)) {
            ...
            if ($acao == 'adicionar') {
                ...
                try {
                    ...
                    // Verificar se as colunas de ID existem na tabela vagas
                    ...
                    $_SESSION['flash_message'] = "Erro ao verificar estrutura da tabela: " . $e->getMessage();
                    $_SESSION['flash_type'] = "danger";
                    return;
                }
